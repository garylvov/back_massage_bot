...
def save_yolo_labels(bboxes, file_path, img_size):
    ...
    Args:
        bboxes: List of bounding boxes in format (x1, y1, x2, y2, class_name)
        file_path: Path to save the labels
        img_size: Tuple of (width, height) for normalization
    """
    with open(file_path, 'w') as f:
        for bbox in bboxes:
            # Extract coordinates - adapting to actual bbox format
            if 'coords' in bbox:
                coords = bbox['coords']
                if len(coords) == 4:  # If coords only has x1,y1,x2,y2
                    x1, y1, x2, y2 = coords
                elif len(coords) == 5:  # If coords has x1,y1,x2,y2,class_name
                    x1, y1, x2, y2, _ = coords
            else:
                # Fall back to direct unpacking if structure is different
                x1, y1, x2, y2 = bbox
            
            name = bbox['name']
            
            # Convert to YOLO format (center_x, center_y, width, height) and normalize
            width = (x2 - x1) / img_size[1]  # grid.shape is (height, width)
            height = (y2 - y1) / img_size[0]
            center_x = (x1 + (x2 - x1) / 2) / img_size[1]
            center_y = (y1 + (y2 - y1) / 2) / img_size[0]
...
class ClusteredNoiseCommand(ImageCommand):
    ...
    def _apply(self, image):
        ...
        for _ in range(num_clusters):
            ...
            # Create the cluster with random shape
            shape_type = np.random.choice(['circle', 'ellipse', 'speckle'])
            ...
            if shape_type == 'circle':
                ...
                a = cluster_size
                b = cluster_size // 2 if cluster_size > 1 else 1
                angle = np.random.uniform(0, 2*np.pi)
                ...
                # Create a speckled pattern within the cluster radius
                for dy in range(-cluster_size, cluster_size+1):
                    for dx in range(-cluster_size, cluster_size+1):
