...
def main():
    ...
    parser.add_argument('--topic', type=str, default='/depth/color/points',
                        help='PointCloud2 topic to subscribe to')
    parser.add_argument('--output-dir', type=str, default='~/pointcloud_plys',
                        help='Directory to save PLY files')
    parser.add_argument('--save-on-timer', action='store_true',
                        help='Save on a timer instead of immediately')
    parser.add_argument('--timer-period', type=float, default=5.0,
                        help='Period (seconds) for timer-based saving')
    parser.add_argument('--frame-limit', type=int, default=1,
                        help='Number of frames to save before shutting down')
